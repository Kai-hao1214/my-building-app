...
st.markdown("""
    ...
    .ref-link { font-size: 0.9rem; color: #718096; margin-top: 20px; }
    </style>
...
if category == "地標與超高層建築":
    ...
    with tab1[0]:
        ...
        st.markdown(
            '<div class="logic-box">1. <b>動態平衡 (TMD)</b>：懸掛於高層的 660 公噸金屬阻尼球，利用物理慣性抵銷大樓擺盪。<br>2. <b>幾幾何流體力學 (Saw-tooth Design)</b>：竹節狀收縮與轉角鋸齒，能有效「打碎」高空氣流，防止形成規律的漩渦（渦流脫離），從根本上降低風力推力。</div>',
            unsafe_allow_html=True)
        st.markdown('**【保留疑問】**')
        st.markdown(
...
# --- 類別二：表演藝術與文化空間 ---
    ...
    with tab2:
        ...
        st.markdown('**【理性求證】**')
        st.markdown(
            '<div class="logic-box">這並非裝飾，而是將牆與柱合一的「壺中居」結構。利用 3D 噴漿混擬土技術，讓牆面本身就具備承重路徑，重力順著曲面滑行到地下，實現無樑柱的大跨度空間。</div>',
            unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.markdown('**【我的故事】**')
        st.markdown(
            '<div class="story-text">巨大的銀色圓球像是從方體中突然長出來一樣，重心極度偏移。這種違反直覺的生長方式，讓我懷疑它在地震頻繁的台灣是否真的站得住腳。</div>',
            unsafe_allow_html=True)
        st.markdown(
    ...
    with tab4:
        ...
        st.markdown('**【我的故事】**')
        st.markdown(
            '<div class="story-text">遠遠看到這座巨大的白色建築，它不像房子，更像一條貼在大地上的魟魚。屋頂是連續的曲面，找不到傳統的轉角。我想知道，這種像布料一樣柔軟的線條，要如何用堅硬的鋼鐵建構出來？</div>',
            unsafe_allow_html=True)
        st.markdown(
        ...
        st.markdown('**【理性求證】**')
        st.markdown(
            '<div class="logic-box">採用類似「造船」的工法。先建構幾何鋼骨骨架，再將數千片弧度各異的鋼板精密拼接。這種單曲率與雙曲率的組合，讓建築外殼本身具備極強的結構剛性。</div>',
            unsafe_allow_html=True)

    with tab5:
        st.markdown('<div class="section-header">高雄流行音樂中心：蜂巢狀的抗拉挑戰</div>', unsafe_allow_html=True)
        st.markdown('**【我的故事：來自網路照片的震撼】**')
        st.markdown(
            '<div class="story-text">在網路上看到高流的照片時，很難不被那座像「蜂巢」一樣的建築吸引。它由無數個六角形組成，表面起伏不平。這種複雜的幾何外殼，讓我好奇這麼多不規則的接點，要如何確保整體的剛性？</div>',
            unsafe_allow_html=True)
        st.markdown(
        ...
        st.markdown('**【理性求證】**')
        st.markdown(
            '<div class="logic-box">採用「空間桁架系統（Space Truss）」。每個六角形透過 3D 座標精確計算出的節點互相傳力，將風壓與自重迅速分散到地面的支撐點。</div>',
            unsafe_allow_html=True)
...
# --- 類別三：地景、橋樑與特殊構造 ---
    ...
    with tab1:
        ...
        st.markdown(
            '<div class="story-text">地形極度不平整。要在這種地方用紅磚疊起一座能跑火車的橋，結構上到底怎麼平衡？</div>',
            unsafe_allow_html=True)
        st.markdown(
            '<div class="logic-box">靠紅磚堆疊的「半圓拱」傳力。透過精確計算各橋墩深度與高度，讓重力依舊能循著拱形軌跡消散。</div>',
            unsafe_allow_html=True)

    with tab2:
        st.markdown('<div class="section-header">故宮南院至美橋：不對稱的力學偏移</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="story-text">身為嘉義人，我對這座橋非常熟悉。它的視覺重心完全偏向一側，在流線型的曲面中，如何克服單側支撐帶來的巨大扭力？</div>',
            unsafe_allow_html=True)
        st.markdown(
            '<div class="logic-box">這是單塔斜張橋，利用內部螺旋鋼樑支撐系統，將不對稱載重轉化為旋轉應力傳導至兩側錨座。</div>',
            unsafe_allow_html=True)

    with tab3:
        st.markdown('<div class="section-header">嘉義高跟鞋教堂：骨架隱形術</div>', unsafe_allow_html=True)
        st.markdown('**【我的故事】**')
        st.markdown('<div class="story-text">玻璃是脆性材料，不能承重。支撐這隻巨大「高跟鞋」的骨架到底躲在哪裡？</div>',
                    unsafe_allow_html=True)
        st.markdown(
    ...
    with tab4:
        ...
        st.markdown(
            '<div class="story-text">只有一座橋塔卻要撐起極長跨距，重心明顯不在中心點，這座橋真的不會傾斜嗎？</div>',
            unsafe_allow_html=True)
        st.markdown(
            '<div class="logic-box">利用「平衡懸臂法」與精確鋼纜張力調整。透過調整左右鋼纜的拉力，將不對稱重力轉化為對橋塔的垂直壓力。</div>',
            unsafe_allow_html=True)